...
// Find minimum risk path across the small cave system
// Using Priority Queue, takes about 100ms to complete
...
/**
 * Calculates the minimum Risk path from the start to the end of the cave, using Dijkstras Alogrithm with Priority Queue
...
function minRisk(input, largeCave = false, logging = false) {
  ...
  // Set start pos distance to 0 cost
  queue.add('0,0', 0);

  // Run until all nodes explored and queue is not empty:
  let numExplored = 0;
  let maxRow = map.length;
  let maxCol = map[0].length;

  while (numExplored < maxRow * maxCol && !queue.isEmpty()) {
    if (numExplored % 1000 === 0 && logging) {
      console.log('Explored so far: ', numExplored, '/', maxRow * maxCol);
    }

    // Find the next node to explore:
    const [node, dist] = queue.remove();
    const [row, col] = node.split(',').map((str) => parseInt(str));

    // If we have reached the ending node, return its cost:
    ...
    for (let inc = -1; inc < 2; inc += 2) {
      ...
      if (newRow >= 0 && newRow < maxRow && !visitMap[newRow][col]) {
        const newNode = `${newRow},${col}`;
        const newDist = dist + map[newRow][col];
        const oldDist = queue.contains(newNode);
        if (oldDist !== -1 && newDist < oldDist) {
          queue.update(newNode, newDist);
        } else if (oldDist === -1) {
          queue.add(newNode, newDist);
        }
      }
      // Check node left/right
      const newCol = col + inc;
      if (newCol >= 0 && newCol < maxCol && !visitMap[row][newCol]) {
        const newNode = `${row},${newCol}`;
        const newDist = dist + map[row][newCol];
        const oldDist = queue.contains(newNode);
        if (oldDist !== -1 && newDist < oldDist) {
          queue.update(newNode, newDist);
        } else if (oldDist === -1) {
...
// More efficient Djikstras Algorithm on a larger cave system
// Takes < 1 second to complete on a graph with 250k nodes
// ~250 x time improvement over using linear search to find the next node to explore (!)
console.log(
